...
from config import (
    ...
    LAMBDA_REC, LR_ENC_DEC, LR_S_DISC, LR_L_DISC,
    SE_REDUCTION, CBAM_REDUCTION, CBAM_SPATIAL_KERNEL,
    LEAKY_RELU_ALPHA, SENSOR_INPUT_DIM
)
...
# ---------------------------------------------------------
# One-Class Adversarial Autoencoder Sensor Model
# CNN + CBAM + BiLSTM
# ---------------------------------------------------------
class HybridAASensor(BaseAnomalyDetector):
    ...
    def __init__(
        ...
        **kwargs
        ...
        # -------------------------------------------------
        # Encoder (CNN + CBAM + BiLSTM)
        # -------------------------------------------------
        self.encoder = tf.keras.Sequential([

            tf.keras.layers.InputLayer(
                input_shape=(input_dim, sequence_length, 1)
            ),
            ...
            # CNN → Sequence
            tf.keras.layers.Reshape((w3, h3 * 128)),
        ...
        self.decoder = tf.keras.Sequential([
            ...
            tf.keras.layers.Cropping2D(((0, crop_h3), (0, crop_w3)))
    ...
    def persistent_weights(self) -> List[tf.Variable]:
        ...
        for opt in [self.enc_dec_opt, self.l_disc_opt, self.s_disc_opt]:
            ...
            if hasattr(opt, 'variables'):
                opt_vars.extend(
                    opt.variables() if callable(opt.variables) else opt.variables
                )
    ...
    def train_func(self, inputs: tf.Tensor) -> Dict[str, tf.Tensor]:
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            rec_loss = tf.reduce_mean(tf.square(inputs - recon))

            sample_d_loss = (
                self.bce(tf.ones_like(d_real), d_real) +
                self.bce(tf.zeros_like(d_fake), d_fake)
            )

            latent_d_loss = (
                self.bce(tf.ones_like(l_real), l_real) +
                self.bce(tf.zeros_like(l_fake), l_fake)
            )
        ...
        self.enc_dec_opt.apply_gradients(
            zip(
                ae_grads,
                self.encoder.trainable_variables +
                self.decoder.trainable_variables
        ...
        s_grads = tape.gradient(
            sample_d_loss,
            self.sample_discriminator.trainable_variables
        )

        self.s_disc_opt.apply_gradients(
            zip(s_grads, self.sample_discriminator.trainable_variables)
        )

        l_grads = tape.gradient(
            latent_d_loss,
            self.latent_discriminator.trainable_variables
        )

        self.l_disc_opt.apply_gradients(
            zip(l_grads, self.latent_discriminator.trainable_variables)
        )
    ...
    @tf.function(
        input_signature=[
            tf.TensorSpec(
                shape=[None, SENSOR_INPUT_DIM, SEQUENCE_LENGTH, 1],
                dtype=tf.float32
            )
        ]
    )
