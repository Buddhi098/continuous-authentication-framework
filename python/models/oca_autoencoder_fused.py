...
from config import (
    ...
    CBAM_REDUCTION, CBAM_SPATIAL_KERNEL, LEAKY_RELU_ALPHA, FUSION_INPUT_DIM, BATCH_SIZE
)
...
# ---------------------------------------------------------
class OneClassAdversarialAutoencoderFused(BaseAnomalyDetector):

    def __init__(self, input_dim=FUSION_INPUT_DIM, sequence_length=SEQUENCE_LENGTH, batch_size=BATCH_SIZE, latent_dim=LATENT_DIM, **kwargs):
    ...
    # ---------------- Training Function ----------------
    @tf.function(input_signature=[tf.TensorSpec(shape=[BATCH_SIZE, FUSION_INPUT_DIM, SEQUENCE_LENGTH, 1], dtype=tf.float32)])
    ...
    # ---------------- Inference ----------------
    @tf.function(input_signature=[tf.TensorSpec(shape=[None, FUSION_INPUT_DIM, SEQUENCE_LENGTH, 1], dtype=tf.float32)])
    def infer_func(self, inputs: tf.Tensor) -> Dict[str, tf.Tensor]:
        ...
        # FIX: Return both the raw reconstruction and the error score.
        # Ensure these string keys exactly match your Android AuthConfigManager values!
        return {
            'reconstruction': recon,   # Android uses this to calculate modelOutputDim
            'anomaly_score': mse       # Android uses this for the actual authentication score
        }
